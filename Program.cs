...
if (string.IsNullOrEmpty(connectionString))
{
    ...
    tempLogger.LogError(errorMessage);
...
catch (Exception ex)
{
    var errorMessage = $"Error configuring DbContext: {ex.Message}";
    var tempLogger = LoggerFactory.Create(config => config.AddConsole()).CreateLogger<Program>();
    tempLogger.LogError(ex, errorMessage);
    throw new InvalidOperationException(errorMessage, ex);
}
...
app.MapPut("/items/{id}", async (int id, Item item, ILogger<Program> logger) =>
    await HandleDbOperationAsync(async db =>
    {
        ...
        // עדכון נכסים של המשימה
        existingItem.Name = item.Name;
        existingItem.IsComplete = item.IsComplete; // תיקון השם ל-IsComplete עם האות הגדולה
        await db.SaveChangesAsync();
        return Results.NoContent();
    }, logger, $"עדכון פריט עם מזהה {id}"));
